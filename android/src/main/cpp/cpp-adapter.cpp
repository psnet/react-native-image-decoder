...
static jsi::ArrayBuffer createZeroCopyArrayBuffer(
    jsi::Runtime& rt,
...
/**
 * Decodes image to RGBA
 */
static jsi::Value decodeImageJSI(
    jsi::Runtime& rt,
    ...
    jsi::Object uint8Array = args[0].asObject(rt);
    jsi::Value bufferVal = uint8Array.getProperty(rt, "buffer");
    ...
    size_t offset = uint8Array.getProperty(rt, "byteOffset").asNumber();
    size_t length = uint8Array.getProperty(rt, "byteLength").asNumber();
    ...
    jsi::Function uint8ArrayConstructor = rt.global().getPropertyAsFunction(rt, "Uint8Array");
    jsi::Object dataArray = uint8ArrayConstructor.callAsConstructor(rt, outBuffer).asObject(rt);
...
Java_com_sepu_imagedecoder_ImageDecoderModule_nativeInstall(JNIEnv* env, jclass, jlong jsContextPtr) {
    ...
    // https://github.com/facebook/hermes/blob/rn/0.81-stable/API/jsi/jsi/jsi.h#L1262
    ...
    rt.global().setProperty(rt, "SepuImageDecoder", rnObj);
}
