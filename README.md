...
Android-only **fast** `JSI C++` image decoder (JS API) zero-copy input/output using `stb_image` for decoding `PNG/JPEG` to `RGBA`.
...
- **Zero-copy input**: reads `Uint8Array.buffer` directly via JSI `ArrayBuffer::data()`/`size()` without copying the compressed bytes.
